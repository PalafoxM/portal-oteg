            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        var buttons = '<button class="ml-1 mr-1 actions-btn" tooltip="Consultar" flow="down" onclick="location.href=\'dt-agencias-viajes/edit/' + row.id + '/\'">';
                        buttons += '<i class="fas fa-edit"></i>';
                        buttons += '</button> ';
                        buttons += '<form method="post" action="dt-agencias-viajes/delete/' + row.id + '/">';
