...
class FuenteInfoParticipacionOrigen(SuperAdminOrAdminMixin, LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = 'Listado de Fuentes de Informacion % Origen'
        context['create_url'] = reverse_lazy('dashboard:fuente_info_origen_create')       
        context['entity'] = 'Fuentes de Informacion % Origen'
...
class FuenteInfoParticipacionOrigenCreate(SuperAdminOrAdminMixin, LoginRequiredMixin, CreateView):    
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            except ParticipacionOrigen.DoesNotExist:
            ...
            if existing_object:
                data = ParticipacionOrigen.objects.filter(ano=ano, destino=destino)

                data_list = list(data.values('ano', 'destino', 'part_visitantes_int', 'part_visitantes_nac','part_visitantes_est'))
                data_list2 = list(form.cleaned_data.values())
                table_html = render_to_string('back/fuente_info_origen/table.html',{'data_list': data_list, 'actual': True, 'data_list2': data_list2})
    ...
    def get_context_data(self, **kwargs):
        ...
        context['list_url'] = reverse_lazy('dashboard:fuente_info_origen')
        context['action'] = 'add'
        return context
...
class OrigenCargaMasivaView(SuperAdminOrAdminMixin, LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(registros_incorrectos) > 0 or len(registros_existentes) > 0:
            ...
            return HttpResponseRedirect(reverse('dashboard:fuente_info_origen'))
    ...
    def procesar_archivo_xlsx(self, archivo):
        ...
        try:
            ...
            for i, row in enumerate(filas):
                ...
                part_visitantes_int = row[2].value
                part_visitantes_nac = row[3].value
                part_visitantes_est = row[4].value

                datos = {
                    'ano':ano ,  
                    'destino':destino , 
                    'part_visitantes_int':part_visitantes_int,
                    'part_visitantes_nac':part_visitantes_nac,
                    'part_visitantes_est':part_visitantes_est, 
                }
                ...
                try:
                    ...
                    existente = ParticipacionOrigen.objects.filter(
                        ano = ano,
                        destino = destino,
                    )
                    if existente.exists():
    ...
    def procesar_archivo_csv(self, archivo):
        ...
        try:
            ...
            for row in datos:
                ...
                part_visitantes_int = row['part_visitantes_int']
                part_visitantes_nac = row['part_visitantes_nac']
                part_visitantes_est = row['part_visitantes_est']
...
class OrigenDescargarArchivoView(SuperAdminOrAdminMixin, LoginRequiredMixin, View):
    ...
    def crear_archivo_excel(self, registros_incorrectos):
        ...
        # Obtener los nombres y verbose_name de los campos del modelo ParticipacionOrigen
        fields = ParticipacionOrigen._meta.get_fields()
        ...
        # Obtener los datos del modelo ParticipacionOrigen
