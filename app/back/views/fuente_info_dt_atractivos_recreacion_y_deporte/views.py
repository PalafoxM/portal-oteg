...
class FuenteInfoDirectorioActivosRecreacionYDeporteCreate (CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if existing_object:
                data = DirectorioActivosRecreacionYDeporte.objects.filter(
                    nombre_de_la_unidad_economica=nombre_de_la_unidad_economica, id_establecimiento=id_establecimiento)
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = 'Crear una fuente Directorio Activos Recreacion Y Deporte'
        context['entity'] = 'Directorio Activos Recreacion Y Deporte'
...
class DirectorioActivosRecreacionYDeporteDescargarArchivoView(View):
    ...
    def crear_archivo_excel(self, registros_incorrectos):
        ...
        # Obtener los nombres de los campos del modelo DirectorioActivosRecreacionYDeporte
        campos = [field.name for field in DirectorioActivosRecreacionYDeporte._meta.get_fields()
                  if field.name != 'id']
